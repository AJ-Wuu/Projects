...
public class BoardManualMode extends JPanel implements ActionListener, KeyListener {

	static int squareSize, boardWidth, boardHeight, squareNumber, charRow, charCol;
	JLabel status;
	...
	int[] xPoints, yPoints;
	boolean isNewRound = true, isFinished = false;
	...
	int[] location; //the coordinates of every square in a shape
	...
	int bump; //the absolute x_index of the square which meets the criteria that cause it to stop moving; bump is always GE 0
	int count = 0; //the number of full lines removed

	public BoardManualMode(FrameManualMode frame) {
		status = frame.status;
		...
		this.setBounds(0, frame.buttonSize, boardWidth, boardHeight);
	...
	private void checkIfRemove() {
		int yMax = 0; //the tallest column of the current shape
		for (int i : heightsInShape.keySet()) {
			yMax = Math.max(yMax, heightsInShape.get(i));
		}
		
		boolean flag = true;
		for (int j=0; j<yMax; j++) {
			int yCurr = y/squareSize - j;
			for (int i=0; i<boardWidth/squareSize; i++) {
				if (squarePiled[i][yCurr] == false) {
					flag = false;
					break;
				}
			}
	...
	private void removeLine(int line) {
		for (int i=0; i<yPainted.length; i++) {
			yPainted[i]++;
		}
		for (int i=0; i<squarePiled.length; i++) {
			for (int j=0; j<squarePiled[0].length; j++) {
				if (squarePiled[i][j] == true) {
					squarePiled[i][j] = false;
					break;
				}
			}
		}
		
		Area lineBlock = new Area(new Rectangle2D.Double(0, line, boardWidth, squareSize));
		for (Area shape : finalShapeG2) {
			shape.exclusiveOr(lineBlock);
		}
	}
