...
public class Frame extends JFrame {
	...
	static Frame frame;
	static BoardAutoMode boardAuto;
	static BoardManualMode boardManual;

	@SuppressWarnings("unused")
	public Frame(Scanner scan, int signal) {
		instructions(scan);
		...
		frame = this;
		boardAuto = new BoardAutoMode(this);
		boardManual = new BoardManualMode(this);
		if (signal == 0) { //(signal == 0) for AutoMode, (signal == 1) for ManualMode
			ButtonsAutoMode buttons = new ButtonsAutoMode(this, status, boardAuto, Width, Height); //there is frame.add() within Buttons() and KeyMoves()
		}
		else {
			ButtonsManualMode buttons = new ButtonsManualMode(this, status, boardManual, Width, Height);
		}
	...
	public static void stop() {
		ButtonsAutoMode.pauseButton.setEnabled(false);
		try {
			Thread.sleep(5);
